...
pal <-colorBin("Greens", domain=final_shape$minority_pct, bins)

labels <- sprintf(
  "<strong>%s</strong><br/>Minority Percentage: %g",
  final_shape$Census_Tra, final_shape$minority_pct
) %>% lapply(htmltools::HTML)
...
leaflet(final_shape) %>%
  ...
  addPolygons(
    ...
    labelOptions = labelOptions(style = list("font-weight" = "normal", padding = "3px 8px"),
                                          textsize = "15px",
                                          direction = "auto")) %>%
  addLegend(pal=pal, values = ~minority_pct, opacity = 0.8, title = "Minority Percentage by Census Tract")
```